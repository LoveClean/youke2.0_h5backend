package com.graduation.project.dao.mapper;

import com.graduation.project.dao.entity.AdGroup;
import org.apache.ibatis.annotations.Param;
import org.apache.ibatis.annotations.Select;
import org.apache.ibatis.annotations.Update;

import java.util.List;

public interface AdGroupMapper {
    @Update("UPDATE tb_ad_group SET del_flag = 1, update_by = #{updateBy}, update_date = now() WHERE id = #{id}")
    int deleteByPrimaryKey(@Param("id") Integer id, @Param("updateBy") String updateBy);

    int insert(AdGroup record);

    int insertSelective(AdGroup record);

    AdGroup selectByPrimaryKey(Integer id);

    @Select("SELECT * FROM tb_ad_group WHERE del_flag = 0 ORDER BY create_date DESC")
    List<AdGroup> selectList();

    @Select("SELECT * FROM tb_ad_group WHERE del_flag = 0 AND name LIKE CONCAT('%',#{name},'%') ORDER BY create_date DESC")
    List<AdGroup> selectListBySearch(@Param("name") String name);

    int updateByPrimaryKeySelective(AdGroup record);

    int updateByPrimaryKey(AdGroup record);
}